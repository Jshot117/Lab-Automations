...
column_probabilites = {
    "Column One" : 0,"Column Two" : 5,"Column Three" : 1,"Column Four" : 0,"Column Five" : 0,"Column Six" : 0,
    "Column Seven" : 0,"Column Eight" : 0,"Column Nine" : 0,"Column Ten" : 0,"Column Eleven" : 0,"Column Twelve" : 0,    
}
...
def run(protocol: protocol_api.ProtocolContext):
    #initalize labware
    wellplate = protocol.load_labware(WELLPLATE_LOADNAME,WELLPLATE_SLOT) ; p20tiprack = protocol.load_labware(P20_TIPRACK_LOADNAME,P20_TIPRACK_SLOT)
    p300tiprack = protocol.load_labware(P300_TIPRACK_LOADNAME,P300_TIPRACK_SLOT) ; tuberack = protocol.load_labware(TUBERACK_LOADNAME,TUBERACK_SLOT)
    ...
    well_plate_volumes = {k:0 for k in wellplate.wells()} #Creates and sets all plate wells to volume of zero
    column_probabilites_list = list(column_probabilites.values()) #Creates a list from dictionary of probabilites
    ...
    #start of commands
    right_pipette.transfer(initial_bacteria_amount,source_well.top(zone_five),patient_zero_well)
